...
- Un Security Group cu port 22 (SSH) si 5000 (Flask) deschise
- O cheie SSH pentru acces la server

### Cerinte

- Terraform instalat
- AWS CLI configurat cu credentiale valide
- O pereche de chei SSH la calea ~/.ssh/id_rsa_proiect si ~/.ssh/id_rsa_proiect.pub
...
# Previzualizare resurse ce vor fi create
terraform plan
...
IP-ul din output este necesar in pasii urmatori pentru Ansible si pentru accesul SSH.
...
Fisierul .github/workflows/deploy.yml se declanseaza la push pe main si:

1. Face checkout la cod
2. Se conecteaza SSH la serverul EC2
3. Creeaza/navigheaza la directorul ~/app
4. Ruleaza docker compose down
5. Curata imaginile vechi (docker image prune -f)
6. Rebuild si pornire: docker compose up --build -d
7. Verifica statusul cu docker compose ps
